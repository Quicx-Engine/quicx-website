...
import { DocsSidebar, type DocsNavGroup } from "@/components/site/docs/DocsSidebar";
...
export function DocsContent() {
  ...
  const navigateTo = useCallback((sectionId: string, elementId: string) => {
    setActiveSectionId(sectionId);
  ...
  }, [activeSectionId, pendingScrollId]);
    ...
    <div className="min-h-screen bg-quicx-bg">
      ...
      <div className="mt-[52px] flex min-h-[calc(100vh-52px)] lg:ml-[260px]">
        <main className="min-w-0 flex-1 max-w-[820px] px-4 py-8 sm:px-8 lg:px-12 lg:py-14">
          ...
          {activeSectionId === "architecture" && <ArchitectureSection />}
          {activeSectionId === "pmad-allocator" && <PmadSection />}
          {activeSectionId === "binary-protocol" && <BinaryProtocolSection />}
          {activeSectionId === "cli-reference" && <CliReferenceSection />}
          {activeSectionId === "java-client" && <JavaClientSection />}
          {activeSectionId === "changelog" && (
            <>
              <ChangelogSection />
              <div className="mt-16">
                <DocsPageFooterCta navigate={navigate} />
              </div>
            </>
          )}
        </main>
        ...
        <div className="hidden xl:block w-[200px] shrink-0 px-5 py-14">
          <DocsTOC entries={activeItem.toc} />
        </div>
      </div>
    </div>
  );
...
class = 1024,8`}
      ...
      <Prose>
        <p>
          ...
          <a href="#pmad-allocator">PMAD</a> initializer. You own every byte of the pool —
...
function DocsPageFooterCta({ navigate }: { navigate: (id: string) => void }) {
  return (
